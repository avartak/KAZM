...
namespace kazm {
    ...
    std::size_t Parser::parseProgramStatement(std::size_t it) throw (Exception) {
        ...
        if (parseToken(T_IF, it+n)) {
            ...
            n++;
            inst = std::make_shared<CallInst>(program, gates["__unitary__"], expv, qidxv);
        }
            ...
            if (qidxv.size() != gate->nqubits) {
                ...
                throw Exception(files.back()->filename, tokens[it+n].line, "Expect two qubits/registers for \'CX\'");
            }
    ...
    std::size_t Parser::parseExp(std::size_t, std::shared_ptr<Expression>&) throw (Exception) {

        return 0;

    }

    std::size_t Parser::parseExpList(std::size_t, std::vector<std::size_t>&) throw (Exception) {
